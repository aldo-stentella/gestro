...
<c:if test="${not empty act}">
	<script type="text/javascript" language="javascript">
		...
		schede.document.forms[0].submit();
		window.close();
	</script>
</c:if>
...
<script language="JavaScript" type="text/JavaScript">
	function elimina(id,nome){
		if(confirm('Confermare l\'eliminazione del documento '+nome)){
			var ref = window.open('DelDocumenti.do?id='+id+'&entita=<%=request.getParameter("entita")%>&tipoDocumentoId=<%=request.getParameter("tipoDocumentoId")%>','popup_eld','height=550,width=850,scrollbars=yes,toolbar=no,status=no,location=no,screenX=100,screenY=20,top=20,left=100','fullscreen=no');
		}
	}
...
<table class="area">
  ...
  <tr>
    ...
    <td width="335"><form enctype="multipart/form-data" method="post" action="Documenti.do">
